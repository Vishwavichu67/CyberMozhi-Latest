...
<body>
  ...
  <div class="print-hint no-print">
    🖨️ A print dialog should open automatically. Choose <strong>"Save as PDF"</strong> as the destination to download this document as a PDF. If it didn't open, press <strong>Ctrl+P</strong> (or Cmd+P on Mac).
  </div>
  ...
  <script>
    // Auto-trigger the print dialog so "Save as PDF" is one click away.
  ...
  // Open in a new tab so the browser's native print-to-PDF flow is immediately available.
  ...
  // Fallback if the popup was blocked: download the HTML file instead.
...
function DownloadButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      title="Opens a print-ready version — choose &quot;Save as PDF&quot; in the print dialog. Allow pop-ups if it doesn't open."
    >
      <Download className="h-3 w-3" />
      {downloading ? 'Opening...' : 'Save as PDF'}
    </button>
  );
}
...
function MessagesList({ messages, isSendingMessage, userId, chatSessionId, onSuggestionSelect }: { messages: Message[]; isSendingMessage: boolean; userId: string; chatSessionId: string | null; onSuggestionSelect: (p: string) => void }) {
  ...
  // Tracks whether the user is currently near the bottom of the scroll area.
  // Used so we only auto-follow new content when they haven't scrolled up
  // to read something earlier — same behavior ChatGPT uses.
  ...
  useEffect(() => {
    ...
    if (isStreamingActive) {
      // While tokens are streaming in: pin to the bottom INSTANTLY, not with
      // a 'smooth' animation. A smooth scroll re-fires and gets interrupted
      // on every single token, which is what caused the visible stutter —
      // this is the same instant-follow approach ChatGPT uses. Only do this
      // if the user hasn't scrolled up to read something earlier.
      if (isNearBottomRef.current) {
        el.scrollTop = el.scrollHeight;
      }
    } else if (isNewMessage) {
      // A message was just sent/completed (not mid-token-stream): one smooth
      // scroll feels natural here, same as ChatGPT's scroll-on-send.
  ...
  return (
    <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-4 md:px-6 space-y-4">
...
export function ChatInterface({ chatSessionId, setChatSessionId, onToggleSidebar, onNewChat }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q,
      snap => {
        // Guard 1: don't overwrite while a message is actively streaming.
        const isStreamingNow = messagesRef.current.some(m => m.isStreaming);
        // Guard 2 (bug fix): Firestore writes happen in the background and can
        // lag behind the optimistic local UI — especially when several
        // messages are sent in quick succession. If an error clears the
        // streaming flag before those background writes land, this listener
        // can fire with a STALE or incomplete snapshot and wipe messages
        // that are already correctly showing on screen. Never let a snapshot
        // regress the visible chat to fewer messages than it already has.
        const wouldLoseMessages = snap.docs.length < messagesRef.current.length;
        if (!isStreamingNow && !wouldLoseMessages) {
      ...
      <footer className="flex-shrink-0 border-t border-border/40 bg-background/95 z-10">
        <div className="p-2">
          ...
          <p className="text-xs text-muted-foreground text-center mt-2">
            CyberMozhi is an AI assistant, not a lawyer. Verify law sections and legal advice with a professional before acting on them.
          </p>
        </div>
      </footer>
    </div>
